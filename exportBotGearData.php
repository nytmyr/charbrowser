    ...
    foreach($rows as $row) {
        ...
        if ($selectedOwner != $row['Owner']) {
            ...
            foreach($rowchars as $rowchar) {
                ...
                if ($rowchar['HastePCT'] > 0 || $rowchar['ClickID'] > 0 || $rowchar['FocusID'] > 0 || $rowchar['WornID'] > 0 || $rowchar['ProcID'] > 0) {
                    $itemSymbols = "[" . $itemname;
                    if ($rowchar['HastePCT'] > 0) {
                        $itemSymbols = $itemSymbols . "H";
                    }
                    if ($rowchar['ClickID'] > 0) {
                        $itemSymbols = $itemSymbols . "C";
                    }
                    if ($rowchar['FocusID'] > 0) {
                        $itemSymbols = $itemSymbols . "F";
                    }
                    if ($rowchar['WornID'] > 0) {
                        $itemSymbols = $itemSymbols . "W";
                    }
                    if ($rowchar['ProcID'] > 0) {
                        $itemSymbols = $itemSymbols . "P";
                    }
                    $itemSymbols = $itemSymbols . "]";
                    if ($showitemnames == "true") {
                        $itemName = $itemSymbols . "" . $itemName;
                    }
                    else {
                        $itemGearScore = $itemSymbols . "" . $itemGearScore;
                    }
                }
        ...
        if ($row['HastePCT'] > 0 || $row['ClickID'] > 0 || $row['FocusID'] > 0 || $row['WornID'] > 0 || $row['ProcID'] > 0) {
            $itemSymbols = "[" . $itemname;
            if ($row['HastePCT'] > 0) {
                $itemSymbols = $itemSymbols . "H";
            }
